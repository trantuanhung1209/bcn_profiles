...
function selectedEndpoints() {
  const checked = new Set(
    [
      ...els.endpointList.querySelectorAll('input[type="checkbox"]:checked'),
...
function selectBy(predicate) {
  for (const input of els.endpointList.querySelectorAll(
    'input[type="checkbox"]',
  )) {
...
async function measureRequest(endpoint) {
  ...
  try {
    ...
    parsedPreview =
      json.message || json.data?.message || JSON.stringify(json).slice(0, 160);
  } catch {
    // keep raw preview
  }
...
async function runEndpoints(endpoints, { ensureLogin = false } = {}) {
  ...
  if (ensureLogin || endpoints.some((ep) => ep.auth || ep.bodyFromForm)) {
    if (!els.email.value.trim() || !els.password.value) {
      setStatus(
        'Nhập email/password trước khi chạy login hoặc endpoint auth.',
        'err',
      );
      return;
    }
  ...
  try {
    if (ensureLogin) {
      ...
      if (!loginResult.ok) {
        setStatus(
          `Login thất bại (${loginResult.status || 'network'}): ${loginResult.preview}`,
          'err',
        );
        state.results = suite;
        renderResults();
    ...
    for (const endpoint of endpoints) {
      ...
      for (let i = 0; i < repeats; i += 1) {
        setStatus(
          `Đang chạy ${endpoint.method} ${endpoint.path} (${i + 1}/${repeats})…`,
        );
    ...
    const allTotals = suite.flatMap((item) =>
      item.runs.map((run) => run.totalMs),
    );
    const avg =
      allTotals.reduce((sum, value) => sum + value, 0) /
      Math.max(allTotals.length, 1);
    setStatus(
...
function wireEvents() {
  els.btnLogin.addEventListener('click', () => {
    ...
    const queue = selected.length
      ? selected
      : ENDPOINTS.filter((ep) => ep.id !== 'logout');
  ...
  els.btnSelectPublic.addEventListener('click', () =>
    selectBy((ep) => !ep.auth),
  );
}
